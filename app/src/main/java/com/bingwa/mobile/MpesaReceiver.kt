...
class MpesaReceiver : BroadcastReceiver() {
    ...
    private fun handleTokenPurchase(context: Context, body: String) {
        ...
        val add = when { amount >= 100 -> 1000; amount >= 50 -> 500; amount >= 10 -> 90; else -> 0 }
        if (add > 0) { tokens.edit().putInt("balance", cur + add).apply(); notify(context, "Tokens Added", "+$add tokens") }
    }
    ...
    private fun handleDataSelling(context: Context, body: String) {
        ...
        if (offer != null && cur >= offer.tokenCost) {
            tokens.edit().putInt("balance", cur - offer.tokenCost).apply()
            context.startService(Intent(context, AutomationService::class.java).apply {
                putExtra("mode", offer.executionMode); putExtra("code", offer.ussdCode); putExtra("phoneNumber", phone)
            })
            notify(context, "USSD Sent", "${offer.name} for $phone")
        } else if (offer != null) notify(context, "Insufficient Tokens", "Need ${offer.tokenCost}, have $cur")
    }
